...
def ThomasTridiagonal(a, b, c, d):
    """
    Solves a tridiagonal linear system Ax = d using the Thomas algorithm.
    - a : array_like, shape (n-1,) Subdiagonal elements (below the main diagonal), i.e., A[i, i-1]
    - b : array_like, shape (n,) Main diagonal elements, i.e., A[i, i]
    - c : array_like, shape (n-1,) Superdiagonal elements (above the main diagonal), i.e., A[i, i+1]
    - d : array_like, shape (n,) Right-hand side vector
...
def ModifyPWCFwds(pwcFwds, deltas):
    """
    Constructs a smooth, area-preserving forward curve by solving a tridiagonal system 
    that ensures continuity of the first derivative between segments.
    """
    ...
    for j in range(1, n-1):
        denom = sum(deltas[j : j + 2])
        a[j-1] = deltas[j+1] / (2.0 * denom)
        c[j] = deltas[j] / (2.0 * denom)
        d[j] = 1.5 * (pwcFwds[j-1] * deltas[j+1] + deltas[j] * pwcFwds[j]) / denom
    fwds = ThomasTridiagonal(a, b, c, d)
    return fwds
...
def BuildCurve(curveDate, handler, 
    ...
    """ Build discount curve using the Area Preserving Quadratic Spline interpolation """
    ...
    deltas = np.append(0.0, np.diff(curveTimes))
    init_guess = None
    ...
    while error > tol and k <= max_iter:
        ...
        init_guess = pwcFwds
        modifiedFwds = ModifyPWCFwds(pwcFwds, deltas)
